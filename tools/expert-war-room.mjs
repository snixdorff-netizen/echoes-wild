...
function buildBacklog() {
  ...
  const items = [
    ...
    { id: 'P0-01', priority: 'P0', category: 'FTUE', title: 'Interactive first-clip tutorial (not text coach)', impact: 10, effort: 'L', owners: ['design_ftue', 'player_ftue'], status: has('guidedCoach') ? 'partial' : 'missing', detail: 'Force: move → hold Listen 2s → Record unlocks → tap ★ card → confetti. No skip on first run.' },
    { id: 'P0-02', priority: 'P0', category: 'Controls', title: 'Remove or defer grip/charge dash mechanic', impact: 9, effort: 'S', owners: ['player_cozy', 'design_ux'], status: 'shipped_bad', detail: 'Reads as different game; scares callers; never taught. Default: walk only; unlock dash in expedition 2.' },
    { id: 'P0-03', priority: 'P0', category: 'UI', title: 'Progressive disclosure — hide top action bar until expedition 2', impact: 9, effort: 'M', owners: ['design_ux', 'player_ftue'], status: 'missing', detail: 'Dock = primary. Top bar: habitat tabs + help only on day 1.' },
    { id: 'P0-04', priority: 'P0', category: 'Visual', title: 'Replace JPEG sprites with vector/svg species silhouettes + glow', impact: 10, effort: 'L', owners: ['player_cozy', 'design_ux'], status: 'partial', detail: 'Glow labels help; sprites still blurry. SVG + palette per species.' },
    { id: 'P0-05', priority: 'P0', category: 'Audio', title: 'Stereo pan + louder proximity falloff when facing caller', impact: 8, effort: 'M', owners: ['design_audio', 'player_puzzle'], status: 'partial', detail: 'Listen cone exists but audio cue of "getting warmer" is weak.' },
    { id: 'P0-06', priority: 'P0', category: 'Identify', title: 'Make spectrogram matter — drag frequency band or tap peaks', impact: 9, effort: 'L', owners: ['player_puzzle', 'design_systems'], status: 'missing', detail: 'Mini spectrogram is static decoration; should hint ID.' },
